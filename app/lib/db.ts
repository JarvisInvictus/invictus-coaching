...
function ensureDB() {
  try {
    ...
    if (!existsSync(DB_PATH)) {
      writeFileSync(DB_PATH, JSON.stringify({ clients: [], weeks: {} }));
    }
  } catch {}
}
...
function writeDB(data: { clients: Client[]; weeks: Record<string, WeekEntry[]> }) {
  ensureDB();
...
export function getClient(id: string): (Client & { weeks: WeekEntry[] }) | null {
  const db = readDB();
  const client = db.clients.find((c: Client) => c.id === id);
...
export function createClient(name: string): Client {
  ...
  if (!db.weeks[client.id]) db.weeks[client.id] = [];

  // Seed Garry Gill's data if first client created is Garry Gill
  if (db.clients.length === 1 && name === "Garry Gill") {
    ...
    const last = GARRY_WEEKS[GARRY_WEEKS.length - 1];
    client.latestBW = last.bodyWeight;
    client.latestDate = last.date;
    client.currentPhase = last.phase;
  }
...
export function updateWeek(clientId: string, date: string, updates: Partial<WeekEntry>): WeekEntry | null {
  ...
  const idx = weeks.findIndex((w: WeekEntry) => w.date === date);
